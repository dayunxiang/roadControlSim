...
import styles from './Areaevaluate.scss'
...
import TimeImg from './time_img.png'
const format = 'HH:mm'
class AreaEvaluate extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      planList: null,
    ...
    this.targetId = this.InterPlanMsg.areaId
    this.resultList = {
      excelParam: '',
      fileNames: '',
      flag: 'false',
    ...
    this.getFileUrl = `/simulation/unitSimResults/getFileNames/${this.targetId}/2` // {targetId}/{taskType}' // 查询当前路口所有方案'
  ...
  getTimeInterval = (fileNames) => {
    getResponseDatas('get', this.timeIntervalUrl + this.targetId + '/' + fileNames + '/2', { fileNames, targetId: this.targetId }).then((res) => {
      const result = res.data
      if (result.code === 200) {
        /* if (!result.content) {
          result.content = { minTime: '00:00', maxTime: '00:00' }
        } */
        if (!result.content) {
          message.error('当前无数据')
  ...
  render() {
    const { timeInterval, videoSrc, unitResult, planList, chartsData } = this.state
    return (
      <div className={styles.areaEvaluateWrapper}>
        <Header areaName={this.InterPlanMsg && this.InterPlanMsg.areaName} hiddenSave={true} />
        <div className={styles.AreaEvaluate}>
          {
            !!this.state.planList &&
            <RowAddPlanList handleVideo={this.handleVideo} planList={planList} getplanListFlag={this.getplanListFlag} />
          }
        ...
        {chartsData ?
          <div className={styles.chartsSearch}>
            <div className={classNames(styles.chartsBox, styles.chartsBoxBg)}>
              ...
              {chartsData.export || <div className={styles.exportBtn} onClick={() => { this.getExportBtn(chartsData) }}>导出</div>}
