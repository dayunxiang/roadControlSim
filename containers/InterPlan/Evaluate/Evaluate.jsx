...
import { DatePicker, Icon, Select, TimePicker, message, Spin } from 'antd'
...
class Evaluate extends React.Component {
  constructor(props) {
    ...
    this.targetId = this.InterPlanMsg.nodeId
    ...
    this.getFileUrl = `/simulation/unitSimResults/getFileNames/${this.targetId}/1` // {targetId}/{taskType}' // 查询当前路口所有方案'
  ...
  getresultList = (bool) => {
    ...
    /* [this.UnitResultUrl, this.unitQueueUrl].forEach((it, ind) => { */
  ...
  getTimeInterval = (fileNames) => {
    getResponseDatas('post', this.timeIntervalUrl, { fileNames, targetId: this.targetId, taskType: 1 }).then((res) => {
      const result = res.data
      if (result.code === 200) {
  ...
  handleVideo = (index) => {
    ...
    if (fileName) {
      getResponseDatas('get', this.videoUrl + fileName + '/1').then((res) => {
        const result = res.data
        if (result.code === 200) {
          // console.log(result.content)
          if (result.content) {
  ...
  render() {
    const { Option } = Select
    const {
      turnArr, directionArr, planList, directiondy, unitResult, timeInterval, chartsData, videoSrc,
    } = this.state
    return (
      <div className={styles.evaluateWrapper}>
        <Header InterName={this.InterPlanMsg.interName} hiddenSave="true" />
        <div className={styles.rowaddlist}>
          {
            planList &&
            <RowAddPlanList planList={planList} getplanListFlag={this.getplanListFlag} handleVideo={this.handleVideo} />
          }
        </div>
